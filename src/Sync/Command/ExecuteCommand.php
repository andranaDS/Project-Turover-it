...
class ExecuteCommand extends ContainerAwareCommand
{
    ...
    protected static $defaultName = 'app:sync:execute';
    ...
    {
        $this->setDescription('Sync JobPostings & Companies from TurnoverIT');
    ...
    protected function fetchItems(InputInterface $input): array
    {
        ...
        $jobPostings = $entitiesToSyncCount > 0 ? $this->getJobPostingsToSync($jobPostingsSyncDate, $entitiesToSyncCount) : [];
    ...
    private function getJobPostingsToSync(?\DateTime $date = null, ?int $limit = null): array
    {
        if (0 === $limit) {
            return [];
        }

        return Arrays::map($this->turnover->getJobPostings($date, $limit), static function (array $data) {
            return Json::encode([
